...
class AnaMenu(QWidget):
    def __init__(self):
        ...
        # ---------------------------
        # moduller klasöründeki exe dosyalarını otomatik ekle
        # ---------------------------
        ...
        # ---------------------------
        # Güncelleme butonu (her durumda ekle)
        # ---------------------------
        btnGuncelle = QPushButton("Güncelle")
        btnGuncelle.setFont(QFont("Arial", 12))
        btnGuncelle.setStyleSheet(
            "background-color: #ffd966; color: black; padding: 10px; border-radius: 10px;"
        )
        btnGuncelle.clicked.connect(self.guncelle_tiklandi)
        grid.addWidget(btnGuncelle, row, col)
    ...
    # ---------------------------
    def guncelle_tiklandi(self):
        if guncelle is not None:
            guncelle.kontrol_ve_guncelle(parent=self)
        else:
            QMessageBox.information(self, "Güncelleme Modülü Eksik",
                                    "Güncelleme modülü bulunamadı!\n"
                                    "Lütfen 'moduller/guncelle.py' dosyasını ekleyin.")
